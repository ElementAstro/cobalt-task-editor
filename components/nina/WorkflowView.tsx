...
import { useSequenceEditorStore, selectCanUndo, selectCanRedo } from '@/lib/nina/store';
...
function WorkflowViewInner() {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Delete' || event.key === 'Backspace') {
        if (selectedItemId) {
          ...
          const findContainerId = (items: typeof sequence.startItems): string | null => {
          ...
          const containerId = findContainerId(sequence.startItems) || 
  ...
  }, [selectedItemId, selectedItemIds, selectedConditionId, selectedTriggerId, deleteItem, deleteCondition, deleteTrigger, duplicateItem, sequence, copySelectedItems, cutSelectedItems, pasteItems, hasClipboard, selectAllItems]);
  ...
  const onNodeClick = useCallback(
    ...
    [selectItem, selectCondition, selectTrigger, toggleItemSelection, selectedItemIds]
  );
      ...
      try {
        ...
        for (let i = startMarkers.length - 1; i >= 0; i--) {
          ...
          if (position.y >= marker.position.y) {
            const markerData = marker.data as { area?: 'start' | 'target' | 'end' };
            if (markerData.area) {
        ...
        const containerNodes = nodes.filter(node => {
          if (node.type !== 'container' && node.type !== 'sequenceItem') return false;
        ...
        if (containerNodes.length > 0) {
          ...
          if (parsed?.type === 'item') {
            const item = useSequenceEditorStore.getState().getItemById(parsed.id);
            if (item && isContainerType(item.type)) {
              targetParentId = parsed.id;
            }
            
            // Also get the area from the target node
            const nodeData = targetNode.data as { area?: 'start' | 'target' | 'end' };
            if (nodeData.area) {
  ...
  const handleContextMenuDelete = useCallback(() => {
    ...
    closeContextMenu();
  }, [contextMenu, deleteItem, deleteCondition, deleteTrigger, sequence, closeContextMenu]);
  ...
  // Move to area
  const handleMoveToArea = useCallback((area: 'start' | 'target' | 'end') => {
    if (!contextMenu || contextMenu.nodeType !== 'item') return;
  ...
  const handleDistribute = useCallback((direction: DistributeType) => {
...
>
  ...
  >
    <Background 
      ...
      size={1} 
      variant={gridSnapEnabled ? BackgroundVariant.Dots : BackgroundVariant.Lines}
    />
    ...
    {showMinimap && (
      <MiniMap
        nodeColor={(node) => {
          ...
          if (node.type === 'areaStart' || node.type === 'areaEnd') return '#22c55e';
          return '#64748b';
        }}
    ...
    {/* Custom Controls Panel */}
        ...
        <Panel position="bottom-left" className="flex gap-1 bg-card/90 backdrop-blur-sm p-1 rounded-lg border border-border shadow-lg">
          {/* Undo/Redo */}
          <Tooltip>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-52">
              <DropdownMenuLabel className="text-xs">{t.workflow?.autoLayout || 'Layout Options'}</DropdownMenuLabel>
              <DropdownMenuSeparator />
          ...
          {selectedItemIds.length >= 2 && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-48">
                <DropdownMenuLabel className="text-xs">{t.workflow?.align || 'Align'}</DropdownMenuLabel>
                <DropdownMenuSeparator />
                ...
                {selectedItemIds.length >= 3 && (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuLabel className="text-xs">{t.workflow?.distribute || 'Distribute'}</DropdownMenuLabel>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-52">
              <DropdownMenuLabel className="text-xs">{t.workflow?.viewSettings || 'View Settings'}</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <div className="flex items-center justify-between px-2 py-1.5">
                <div className="flex items-center gap-2">
              ...
              <div className="flex items-center justify-between px-2 py-1.5">
                <div className="flex items-center gap-2">
                  {showAreaBackgrounds ? <Eye className="w-4 h-4 text-muted-foreground" /> : <EyeOff className="w-4 h-4 text-muted-foreground" />}
          ...
          {selectedItemIds.length > 1 && (
            <div className="flex items-center gap-1 px-2 text-xs text-muted-foreground">
              <span>{selectedItemIds.length} {t.workflow?.selected || 'selected'}</span>
              <Button
                variant="ghost"
                size="sm"
        ...
        {/* Info Panel */}
        <Panel position="top-right" className="bg-card/90 backdrop-blur-sm p-2 rounded-lg border border-border shadow-lg">
        ...
        >
          {contextMenu.nodeType === 'item' && (
            <>
              ...
              <div className="px-2 py-1 text-xs text-muted-foreground">{t.workflow.moveToStart}</div>
              <button
                className="flex items-center gap-2 w-full px-2 py-1.5 text-sm rounded-sm hover:bg-accent hover:text-accent-foreground"
        ...
        >
          ...
          >
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span>{t.shortcuts.undo}</span><kbd className="px-2 py-0.5 bg-muted rounded text-xs">Ctrl+Z</kbd></div>
              <div className="flex justify-between"><span>{t.shortcuts.redo}</span><kbd className="px-2 py-0.5 bg-muted rounded text-xs">Ctrl+Y</kbd></div>
              <div className="flex justify-between"><span>{t.shortcuts.delete}</span><kbd className="px-2 py-0.5 bg-muted rounded text-xs">Delete</kbd></div>
              <div className="flex justify-between"><span>{t.shortcuts.duplicate}</span><kbd className="px-2 py-0.5 bg-muted rounded text-xs">Ctrl+D</kbd></div>
              <div className="flex justify-between"><span>Multi-select</span><kbd className="px-2 py-0.5 bg-muted rounded text-xs">Ctrl+Click</kbd></div>
              <div className="flex justify-between"><span>Box select</span><kbd className="px-2 py-0.5 bg-muted rounded text-xs">Drag</kbd></div>
            </div>
            <button 
