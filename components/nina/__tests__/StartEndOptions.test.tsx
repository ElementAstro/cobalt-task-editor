...
describe('Start/End Options Store', () => {
  describe('Start Options', () => {
    it('should have default start options', () => {
      ...
      expect(typeof store.sequence.startOptions.coolCameraAtSequenceStart).toBe('boolean');
      expect(typeof store.sequence.startOptions.coolCameraTemperature).toBe('number');
    });
  ...
  describe('End Options', () => {
    it('should have default end options', () => {
      ...
      expect(typeof store.sequence.endOptions.warmCamAtSequenceEnd).toBe('boolean');
      expect(typeof store.sequence.endOptions.parkMountAtSequenceEnd).toBe('boolean');
    });
  ...
  describe('Options Structure', () => {
    it('should have coolCameraDuration in start options', () => {
      const store = useSimpleSequenceStore.getState();
      expect(typeof store.sequence.startOptions.coolCameraDuration).toBe('number');
    });

    it('should have warmCameraDuration in end options', () => {
      const store = useSimpleSequenceStore.getState();
      expect(typeof store.sequence.endOptions.warmCameraDuration).toBe('number');
    });
  });
});
