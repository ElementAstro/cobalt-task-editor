...
export function decimalToRA(decimal: number): {
  hours: number;
  minutes: number;
  seconds: number;
} {
...
export function decimalToDec(decimal: number): {
  degrees: number;
  minutes: number;
  seconds: number;
  negative: boolean;
...
export function insertItemAt(
  ...
  return items.map((item) => {
    ...
    if (item.items) {
      return {
        ...item,
        items: insertItemAt(item.items, parentId, index, newItem),
      };
    }
    return item;
  });
...
export function cloneSequenceItem(
