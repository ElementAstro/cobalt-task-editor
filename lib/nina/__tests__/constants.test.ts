...
describe("Constants", () => {
  ...
  describe("SEQUENCE_ITEMS", () => {
    ...
    it("should include container items", () => {
      const containers = SEQUENCE_ITEMS.filter(
        (item) => item.category === CATEGORIES.CONTAINER,
      );
      expect(containers.length).toBeGreaterThan(0);
      expect(containers.some((c) => c.name === "Sequential Container")).toBe(
        true,
      );
      expect(containers.some((c) => c.name === "Parallel Container")).toBe(
        true,
      );
    });

    it("should include camera items", () => {
      const cameraItems = SEQUENCE_ITEMS.filter(
        (item) => item.category === CATEGORIES.CAMERA,
      );
    ...
    it("should include imaging items", () => {
      const imagingItems = SEQUENCE_ITEMS.filter(
        (item) => item.category === CATEGORIES.IMAGING,
      );
  ...
  describe("TRIGGER_ITEMS", () => {
    ...
    it("should include meridian flip trigger", () => {
      const meridianFlip = TRIGGER_ITEMS.find(
        (t) => t.name === "Meridian Flip",
      );
  ...
  describe("ERROR_BEHAVIORS", () => {
    it("should have error behavior options", () => {
      ...
      expect(ERROR_BEHAVIORS.some((b) => b.label === "Continue on Error")).toBe(
        true,
      );
      expect(ERROR_BEHAVIORS.some((b) => b.label === "Abort on Error")).toBe(
        true,
      );
    });
  });
  ...
  describe("EXECUTION_STRATEGIES", () => {
    it("should have execution strategies", () => {
      ...
      expect(EXECUTION_STRATEGIES.some((s) => s.label === "Sequential")).toBe(
        true,
      );
      expect(EXECUTION_STRATEGIES.some((s) => s.label === "Parallel")).toBe(
        true,
      );
    });
  });
...
describe("Helper Functions", () => {
  describe("getItemDefinition", () => {
    it("should find sequence item by type", () => {
      const coolCamera = getItemDefinition(
    ...
    it("should find condition by type", () => {
      const loopCondition = getItemDefinition(
    ...
    it("should find trigger by type", () => {
      const meridianFlip = getItemDefinition(
