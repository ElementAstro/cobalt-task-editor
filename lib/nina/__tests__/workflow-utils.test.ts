...
const createTestSequence = (overrides: Partial<EditorSequence> = {}): EditorSequence => ({
  id: 'test-seq',
  title: 'Test Sequence',
  startItems: [],
  targetItems: [],
...
describe('workflow-utils', () => {
  ...
  describe('sequenceToFlow', () => {
    ...
    it('should create edges between sequential items', () => {
      ...
      const itemEdge = edges.find(e => 
        e.source === 'node-item-1' && e.target === 'node-item-2'
      );
    ...
    it('should create edges between areas', () => {
      ...
      const areaEdges = edges.filter(e => 
        e.source.includes('area-end') && e.target.includes('area-start')
      );
  ...
  describe('getItemTypeColor', () => {
    it('should return yellow for DeepSkyObject', () => {
      expect(getItemTypeColor('NINA.Sequencer.Container.DeepSkyObjectContainer, NINA.Sequencer'))
        .toBe('#eab308');
    });

    it('should return blue for Sequential', () => {
      expect(getItemTypeColor('NINA.Sequencer.Container.SequentialContainer, NINA.Sequencer'))
        .toBe('#3b82f6');
    });

    it('should return purple for Parallel', () => {
      expect(getItemTypeColor('NINA.Sequencer.Container.ParallelContainer, NINA.Sequencer'))
        .toBe('#a855f7');
    });

    it('should return orange for Cool/Warm camera', () => {
      expect(getItemTypeColor('NINA.Sequencer.SequenceItem.Camera.CoolCamera, NINA.Sequencer'))
        .toBe('#f97316');
    });

    it('should return green for Exposure', () => {
      expect(getItemTypeColor('NINA.Sequencer.SequenceItem.Imaging.TakeExposure, NINA.Sequencer'))
        .toBe('#22c55e');
    });
  ...
  describe('autoLayoutNodes', () => {
    it('should layout nodes with dagre', () => {
      const nodes: Node[] = [
        { id: 'node-1', type: 'sequenceItem', position: { x: 0, y: 0 }, data: {} },
        { id: 'node-2', type: 'sequenceItem', position: { x: 0, y: 0 }, data: {} },
      ];
      const edges: Edge[] = [
  ...
  describe('alignNodes', () => {
    const createTestNodes = (): Node[] => [
      { id: 'node-1', type: 'sequenceItem', position: { x: 100, y: 50 }, data: {} },
      { id: 'node-2', type: 'sequenceItem', position: { x: 200, y: 100 }, data: {} },
      { id: 'node-3', type: 'sequenceItem', position: { x: 150, y: 200 }, data: {} },
    ];
    ...
    it('should align nodes to right', () => {
      ...
      expect(alignedNodes.every(n => n.position.x === alignedNodes[0].position.x)).toBe(true);
    });
    ...
    it('should align nodes to bottom', () => {
      ...
      const alignedNodes = result.filter(n => selectedIds.includes(n.id));
      expect(alignedNodes.every(n => n.position.y === alignedNodes[0].position.y)).toBe(true);
    });
  ...
  describe('distributeNodes', () => {
    const createTestNodes = (): Node[] => [
      { id: 'node-1', type: 'sequenceItem', position: { x: 0, y: 0 }, data: {} },
      { id: 'node-2', type: 'sequenceItem', position: { x: 100, y: 50 }, data: {} },
      { id: 'node-3', type: 'sequenceItem', position: { x: 300, y: 100 }, data: {} },
    ];
    ...
    it('should not modify unselected nodes', () => {
      const nodes = [
        ...createTestNodes(),
        { id: 'node-4', type: 'sequenceItem', position: { x: 500, y: 500 }, data: {} },
      ];
      const selectedIds = ['node-1', 'node-2', 'node-3'];
      const result = distributeNodes(nodes, selectedIds, 'horizontal');
  ...
  describe('getSelectionBounds', () => {
    ...
    it('should only consider selected nodes', () => {
      const nodes: Node[] = [
        { id: 'node-1', type: 'sequenceItem', position: { x: 100, y: 50 }, data: {} },
        { id: 'node-2', type: 'sequenceItem', position: { x: 500, y: 500 }, data: {} },
      ];
