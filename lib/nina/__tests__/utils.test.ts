...
describe('Item Creation', () => {
  describe('createSequenceItem', () => {
    it('should create a sequence item with default values', () => {
      const item = createSequenceItem('NINA.Sequencer.SequenceItem.Camera.CoolCamera, NINA.Sequencer');
      
      expect(item.id).toBeTruthy();
      expect(item.type).toBe('NINA.Sequencer.SequenceItem.Camera.CoolCamera, NINA.Sequencer');
      expect(item.status).toBe('CREATED');
    });

    it('should create a container item with items array', () => {
      const item = createSequenceItem('NINA.Sequencer.Container.SequentialContainer, NINA.Sequencer');
  ...
  describe('createCondition', () => {
    it('should create a condition with default values', () => {
      const condition = createCondition('NINA.Sequencer.Conditions.LoopCondition, NINA.Sequencer');
      
      expect(condition.id).toBeTruthy();
      expect(condition.type).toBe('NINA.Sequencer.Conditions.LoopCondition, NINA.Sequencer');
    });
  ...
  describe('createTrigger', () => {
    it('should create a trigger with default values', () => {
      const trigger = createTrigger('NINA.Sequencer.Trigger.MeridianFlip.MeridianFlipTrigger, NINA.Sequencer');
      
      expect(trigger.id).toBeTruthy();
      expect(trigger.type).toBe('NINA.Sequencer.Trigger.MeridianFlip.MeridianFlipTrigger, NINA.Sequencer');
...
describe('Target Creation', () => {
  describe('createEmptyTarget', () => {
    it('should create an empty target', () => {
      ...
      expect(target.dec).toEqual({ degrees: 0, minutes: 0, seconds: 0, negative: false });
  ...
  describe('createTarget', () => {
    it('should create a target with specified values', () => {
      ...
      expect(target.dec).toEqual({ degrees: 41, minutes: 16, seconds: 9, negative: false });
...
describe('Coordinate Utilities', () => {
  ...
  describe('formatDec', () => {
    it('should format positive Dec correctly', () => {
      const formatted = formatDec({ degrees: 45, minutes: 30, seconds: 15.5, negative: false });
      expect(formatted).toBe('+45° 30\' 15.5"');
    });

    it('should format negative Dec correctly', () => {
      const formatted = formatDec({ degrees: 45, minutes: 30, seconds: 15.5, negative: true });
      expect(formatted).toBe('-45° 30\' 15.5"');
    });
  });
});
...
describe('Tree Utilities', () => {
  ...
  describe('updateItemById', () => {
    ...
    it('should update nested item', () => {
      const tree = createTestTree();
      const result = updateItemById(tree, 'item1-1', { name: 'Updated Item 1-1' });
...
describe('Type Name Utilities', () => {
  describe('getShortTypeName', () => {
    it('should extract short type name', () => {
      expect(getShortTypeName('NINA.Sequencer.SequenceItem.Camera.CoolCamera, NINA.Sequencer'))
        .toBe('CoolCamera');
    });

    it('should handle container types', () => {
      expect(getShortTypeName('NINA.Sequencer.Container.SequentialContainer, NINA.Sequencer'))
  ...
  describe('getTypeCategory', () => {
    it('should extract category from type', () => {
      ...
      expect(getTypeCategory('NINA.Sequencer.SequenceItem.Camera.CoolCamera, NINA.Sequencer'))
        .toBe('CoolCamera');
    });

    it('should handle container types', () => {
      expect(getTypeCategory('NINA.Sequencer.Container.SequentialContainer, NINA.Sequencer'))
        .toBe('SequentialContainer');
    });
