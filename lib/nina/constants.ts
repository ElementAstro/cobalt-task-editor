...
import { ExecutionStrategyType, ImageType, InstructionErrorBehavior } from './types';
  ...
  },
  {
    ...
    defaultValues: { 
      ...
      Target: {
        ...
        Rotation: 0,
        InputCoordinates: {
          RAHours: 0, RAMinutes: 0, RASeconds: 0,
          DecDegrees: 0, DecMinutes: 0, DecSeconds: 0,
        }
      }
    },
  },
...
export function getItemDefinition(type: string): ItemDefinition | undefined {
  return SEQUENCE_ITEMS.find(item => item.type === type) ||
...
export const ERROR_BEHAVIORS = [
  { value: InstructionErrorBehavior.ContinueOnError, label: 'Continue on Error' },
  { value: InstructionErrorBehavior.AbortOnError, label: 'Abort on Error' },
  { value: InstructionErrorBehavior.SkipInstructionSetOnError, label: 'Skip Instruction Set' },
  { value: InstructionErrorBehavior.SkipToSequenceEndInstructions, label: 'Skip to End' },
];
