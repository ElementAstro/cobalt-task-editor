...
interface SimpleSequenceActions {
  ...
  updateTargetCoordinates: (targetId: string, coordinates: Partial<Coordinates>) => void;
  ...
  updateExposure: (targetId: string, exposureId: string, updates: Partial<SimpleExposure>) => void;
  ...
  persist(
    immer((set, get) => ({
      ...
      removeTarget: (targetId) => {
        set((state) => {
          const index = state.sequence.targets.findIndex((t) => t.id === targetId);
          if (index === -1) return;
          ...
          if (state.sequence.selectedTargetId === targetId) {
            if (state.sequence.targets.length > 0) {
              const newIndex = Math.min(index, state.sequence.targets.length - 1);
              state.sequence.selectedTargetId = state.sequence.targets[newIndex].id;
            } else {
          ...
          if (state.sequence.activeTargetId === targetId) {
            state.sequence.activeTargetId = state.sequence.targets[0]?.id || null;
          }
      ...
      moveTargetDown: (targetId) => {
        set((state) => {
          const index = state.sequence.targets.findIndex((t) => t.id === targetId);
          if (index === -1 || index >= state.sequence.targets.length - 1) return;
      ...
      importFromCSV: async (csvContent) => {
        ...
        try {
          ...
          const isTelescopius = headers.includes('pane') || headers.includes('familiar name');
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            if (isTelescopius) {
              if (row['pane']) {
                ...
                raStr = row['ra'] || '';
                decStr = row['dec'] || '';
                positionAngle = parseFloat(row['position angle (east)'] || '0') || 0;
              } else if (row['familiar name'] || row['catalogue entry']) {
                name = row['familiar name'] || row['catalogue entry'];
                raStr = row['right ascension'] || row['right ascension (j2000)'] || '';
                decStr = row['declination'] || row['declination (j2000)'] || '';
                positionAngle = parseFloat(row['position angle (east)'] || '0') || 0;
              }
            } else {
              // Standard CSV format
              name = row['name'] || row['target'] || row['object'] || `Target ${i}`;
              raStr = row['ra'] || row['right ascension'] || '';
              decStr = row['dec'] || row['declination'] || '';
              positionAngle = parseFloat(row['pa'] || row['position angle'] || '0') || 0;
            }
          ...
          set((state) => {
            state.isImporting = false;
            state.lastError = error instanceof Error ? error.message : 'Import failed';
          });
          return false;
        }
      },
      ...
      importFromJSON: (jsonContent) => {
        try {
          ...
          set((state) => {
            ...
            if (data.startOptions) Object.assign(state.sequence.startOptions, data.startOptions);
            if (data.endOptions) Object.assign(state.sequence.endOptions, data.endOptions);
            if (data.estimatedDownloadTime) state.sequence.estimatedDownloadTime = data.estimatedDownloadTime;
      ...
      exportToTargetSet: () => {
        ...
        const targetSet: TargetSetExport = {
          ...
          StartOptions: {
            CoolCameraAtSequenceStart: sequence.startOptions.coolCameraAtSequenceStart,
            ...
            UnparkMountAtSequenceStart: sequence.startOptions.unparkMountAtSequenceStart,
          ...
          Targets: sequence.targets.map((target): CaptureSequenceExport => ({
            ...
            AutoFocusAfterTemperatureChange: target.autoFocusAfterTemperatureChange,
            AutoFocusAfterTemperatureChangeAmount: target.autoFocusAfterTemperatureChangeAmount,
            AutoFocusAfterHFRChange: target.autoFocusAfterHFRChange,
            AutoFocusAfterHFRChangeAmount: target.autoFocusAfterHFRChangeAmount,
            Items: target.exposures.map((exp): CaptureSequenceItemExport => ({
              Enabled: exp.enabled,
              ExposureTime: exp.exposureTime,
              ImageType: exp.imageType,
              FilterType: exp.filter ? { Name: exp.filter.name, Position: exp.filter.position } : null,
      ...
      exportToXML: () => {
        ...
        const targetsXml = sequence.targets.map((target) => {
          // Convert coordinates to decimal degrees for NINA
          const raDegrees = (target.coordinates.raHours + 
            target.coordinates.raMinutes / 60 + 
            target.coordinates.raSeconds / 3600) * 15; // RA hours to degrees
          const decDegrees = (target.coordinates.decDegrees + 
            target.coordinates.decMinutes / 60 + 
            target.coordinates.decSeconds / 3600) * (target.coordinates.negativeDec ? -1 : 1);
          
          const exposuresXml = target.exposures.map((exp) => `
      <CaptureSequence>
...
<ArrayOfCaptureSequenceList xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">${targetsXml}
      ...
      copyExposuresToAllTargets: (sourceTargetId) => {
        const state = get();
        const sourceTarget = state.sequence.targets.find((t) => t.id === sourceTargetId);
      ...
      copyExposuresToTarget: (sourceTargetId, destTargetId) => {
        ...
        set((innerState) => {
          const destTarget = innerState.sequence.targets.find((t) => t.id === destTargetId);
      ...
      calculateETAs: () => {
        set((state) => {
          ...
          state.sequence.overallEndTime = new Date(Date.now() + totalDuration * 1000);
      ...
      undo: () => {
        set((state) => {
          if (state.historyIndex > 0) {
            state.historyIndex--;
            state.sequence = JSON.parse(JSON.stringify(state.history[state.historyIndex]));
          }
        });
      },
      ...
      getSelectedTarget: () => {
        const state = get();
        return state.sequence.targets.find((t) => t.id === state.sequence.selectedTargetId) || null;
      },
      ...
      getTotalExposureCount: () => {
        return get().sequence.targets.reduce(
          (total, target) => total + target.exposures.reduce((t, e) => t + e.totalCount, 0),
          0
        );
      },
      ...
      getTotalRemainingExposures: () => {
        return get().sequence.targets.reduce(
          (total, target) =>
            total + target.exposures.reduce((t, e) => t + Math.max(0, e.totalCount - e.progressCount), 0),
          0
        );
      },
    })),
...
export const selectTargets = (state: SimpleSequenceStore) => state.sequence.targets;
export const selectSelectedTargetId = (state: SimpleSequenceStore) => state.sequence.selectedTargetId;
export const selectActiveTargetId = (state: SimpleSequenceStore) => state.sequence.activeTargetId;
export const selectStartOptions = (state: SimpleSequenceStore) => state.sequence.startOptions;
export const selectEndOptions = (state: SimpleSequenceStore) => state.sequence.endOptions;
export const selectIsRunning = (state: SimpleSequenceStore) => state.sequence.isRunning;
export const selectIsDirty = (state: SimpleSequenceStore) => state.sequence.isDirty;
export const selectCanUndo = (state: SimpleSequenceStore) => state.historyIndex > 0;
export const selectCanRedo = (state: SimpleSequenceStore) => state.historyIndex < state.history.length - 1;
