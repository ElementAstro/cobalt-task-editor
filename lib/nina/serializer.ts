...
function createNINARootContainer(
  ...
  return {
    ...
    Triggers: {
      ...
      $values: sequence.globalTriggers.map((t) =>
        convertEditorTriggerToNINA(t, rootId),
      ),
    },
    Parent: null,
  };
...
function convertEditorItemToNINA(
  ...
  if (isContainer && item.items) {
    ...
    container.Items = {
      ...
      $type:
        "System.Collections.ObjectModel.ObservableCollection`1[[NINA.Sequencer.SequenceItem.ISequenceItem, NINA.Sequencer]], System.ObjectModel",
      $values: item.items.map((child) =>
        convertEditorItemToNINA(child, itemId),
      ),
    };
    ...
    container.Conditions = {
      ...
      $type:
        "System.Collections.ObjectModel.ObservableCollection`1[[NINA.Sequencer.Conditions.ISequenceCondition, NINA.Sequencer]], System.ObjectModel",
      $values: (item.conditions || []).map((c) =>
        convertEditorConditionToNINA(c, itemId),
      ),
    };
    ...
    container.Triggers = {
      ...
      $type:
        "System.Collections.ObjectModel.ObservableCollection`1[[NINA.Sequencer.Trigger.ISequenceTrigger, NINA.Sequencer]], System.ObjectModel",
      $values: (item.triggers || []).map((t) =>
        convertEditorTriggerToNINA(t, itemId),
      ),
    };
...
function convertEditorConditionToNINA(
...
function convertEditorTriggerToNINA(
...
function importRootContainer(root: ISequenceRootContainer): EditorSequence {
  ...
  const startItems = items[0]
    ? convertNINAContainerItems(items[0] as ISequenceContainer)
    : [];
  const targetItems = items[1]
    ? convertNINAContainerItems(items[1] as ISequenceContainer)
    : [];
  const endItems = items[2]
    ? convertNINAContainerItems(items[2] as ISequenceContainer)
    : [];

  const globalTriggers = (root.Triggers?.$values || []).map(
    convertNINATriggerToEditor,
  );
...
function convertNINAContainerItems(
  container: ISequenceContainer,
): EditorSequenceItem[] {
...
function convertNINAItemToEditor(item: ISequenceItem): EditorSequenceItem {
  ...
  if (isContainer) {
    ...
    editorItem.items = (container.Items?.$values || []).map(
      convertNINAItemToEditor,
    );
    editorItem.conditions = (container.Conditions?.$values || []).map(
      convertNINAConditionToEditor,
    );
    editorItem.triggers = (container.Triggers?.$values || []).map(
      convertNINATriggerToEditor,
    );
...
function convertNINAConditionToEditor(
...
function convertNINATriggerToEditor(trigger: ISequenceTrigger): EditorTrigger {
  ...
  const triggerItems =
    trigger.TriggerRunner?.Items?.$values?.map(convertNINAItemToEditor) || [];
...
export function exportTemplateToNINA(
  items: EditorSequenceItem[],
  name: string,
): string {
  resetNinaIds();
...
export function validateNINAJson(jsonString: string): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
