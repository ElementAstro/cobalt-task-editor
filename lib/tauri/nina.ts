 ...
 */
export async function exportToNinaJson(sequence: EditorSequence): Promise<string> {
  if (isTauri()) {
 ...
 */
export async function importFromNinaJson(json: string): Promise<EditorSequence> {
  if (isTauri()) {
 ...
 */
export async function validateNinaFormat(json: string): Promise<{ valid: boolean; errors: string[] }> {
  if (isTauri()) {
    try {
 ...
 */
export async function loadNinaSequenceFile(path: string): Promise<EditorSequence> {
  if (isTauri()) {
...
export async function isNinaContainerType(typeStr: string): Promise<boolean> {
  ...
  return typeStr.includes('Container')
