...
describe('Optimizer Module', () => {
  const createTestTarget = (name: string, raHours: number = 0): SimpleTarget => ({
    id: `target-${name}`,
    name,
  ...
  describe('optimizeTargetOrder (browser fallback)', () => {
    it('should return optimization result', async () => {
      ...
      const result = await optimizeTargetOrder(sequence, testLocation, testDate);
    ...
    it('should accept different strategies', async () => {
      ...
      for (const strategy of strategies) {
        const result = await optimizeTargetOrder(sequence, testLocation, testDate, strategy);
        expect(result.success).toBe(true);
      }
    });
  });
  ...
  describe('detectScheduleConflicts (browser fallback)', () => {
    it('should return conflict result', async () => {
      const sequence = createTestSequence();
      const result = await detectScheduleConflicts(sequence, testLocation, testDate);
  ...
  describe('getTargetScheduleInfo (browser fallback)', () => {
    it('should return schedule info for all targets', async () => {
      const sequence = createTestSequence();
      const info = await getTargetScheduleInfo(sequence, testLocation, testDate);
  ...
  describe('batchCalculateVisibility (browser fallback)', () => {
    it('should calculate visibility for all targets', async () => {
      const sequence = createTestSequence();
      const results = await batchCalculateVisibility(sequence, testLocation, testDate);
  ...
  describe('validateSequenceForDate (browser fallback)', () => {
    it('should return validation report', async () => {
      const sequence = createTestSequence();
      const report = await validateSequenceForDate(sequence, testLocation, testDate);
  ...
  describe('estimateSessionTime (browser fallback)', () => {
    it('should estimate session time', async () => {
      const sequence = createTestSequence();
      const estimate = await estimateSessionTime(sequence, testLocation, testDate);
    ...
    it('should include slew time when requested', async () => {
      ...
      const withSlew = await estimateSessionTime(sequence, testLocation, testDate, true);
      const withoutSlew = await estimateSessionTime(sequence, testLocation, testDate, false);
