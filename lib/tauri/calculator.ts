 ...
 */
export async function calculateSequenceRuntime(
  ...
  for (const target of sequence.targets) {
    ...
    for (const exp of target.exposures) {
      if (exp.enabled) {
        ...
        total +=
          remaining * (exp.exposureTime + sequence.estimatedDownloadTime);
      }
    }
  }
  return total;
 ...
 */
export async function calculateSequenceEtas(
  sequence: SimpleSequence,
...
export async function calculateAngularSeparation(
  ...
  // Browser fallback
  const ra1 =
    ((coord1.raHours + coord1.raMinutes / 60 + coord1.raSeconds / 3600) *
      15 *
      Math.PI) /
    180;
  const dec1 =
    ((coord1.decDegrees + coord1.decMinutes / 60 + coord1.decSeconds / 3600) *
      (coord1.negativeDec ? -1 : 1) *
      Math.PI) /
    180;
  const ra2 =
    ((coord2.raHours + coord2.raMinutes / 60 + coord2.raSeconds / 3600) *
      15 *
      Math.PI) /
    180;
  const dec2 =
    ((coord2.decDegrees + coord2.decMinutes / 60 + coord2.decSeconds / 3600) *
      (coord2.negativeDec ? -1 : 1) *
      Math.PI) /
    180;
 ...
 */
export async function raToDecimal(
  hours: number,
  minutes: number,
  seconds: number,
): Promise<number> {
...
export async function decToDecimal(
  ...
  if (isTauri()) {
    return invoke<number>("dec_to_decimal", {
      degrees,
      minutes,
      seconds,
      negative,
...
export async function calculateAltitude(
  ...
  if (isTauri()) {
    return invoke<number>("calculate_altitude", {
      raHours,
      decDegrees,
      latitude,
      longitude,
...
export async function isAboveHorizon(
  ...
  const altitude = await calculateAltitude(
    raHours,
    decDegrees,
    latitude,
    longitude,
 ...
 */
export async function calculateMoonIllumination(
  datetime?: string,
): Promise<number> {
  if (isTauri()) {
 ...
 * Format RA
 */
export async function formatRa(
  hours: number,
  minutes: number,
  seconds: number,
): Promise<string> {
...
export async function formatDec(
  ...
  if (isTauri()) {
    return invoke<string>("format_dec", {
      degrees,
      minutes,
      seconds,
      negative,
