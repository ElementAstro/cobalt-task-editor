...
use crate::services::astronomy::{
    ObserverLocation, VisibilityWindow, calculate_visibility_window,
...
pub fn optimize_sequence(
    ...
    let mut target_info: Vec<(String, &SimpleTarget, VisibilityWindow, f64)> = sequence.targets
        .iter()
        .map(|target| {
    ...
    match strategy {
        ...
        OptimizationStrategy::VisibilityDuration => {
            target_info.sort_by(|a, b| b.2.duration_hours.partial_cmp(&a.2.duration_hours).unwrap());
        ...
        OptimizationStrategy::Combined => {
            ...
            target_info.sort_by(|a, b| {
                let score_a = a.2.max_altitude / 90.0 * 30.0 + a.3 * 0.5 + a.2.duration_hours / 12.0 * 20.0;
                let score_b = b.2.max_altitude / 90.0 * 30.0 + b.3 * 0.5 + b.2.duration_hours / 12.0 * 20.0;
    ...
    for (_id, target, window, _) in &target_info {
        if !window.is_visible {
            warnings.push(format!("Target '{}' is not visible on this date", target.target_name));
        }
    }
...
pub fn detect_conflicts(
    ...
    let target_info: Vec<(String, String, VisibilityWindow, f64)> = sequence.targets
        .iter()
        .map(|target| {
            let window = calculate_visibility_window(&target.coordinates, location, date, 20.0);
            let runtime = target.runtime(download_time);
            (target.id.clone(), target.target_name.clone(), window, runtime)
        })
        .collect();
        ...
        if *runtime1 > window1.duration_hours * 3600.0 {
            conflicts.push(ScheduleConflict {
                ...
                description: format!(
                    ...
                    name1, runtime1 / 3600.0, window1.duration_hours
                ),
            });
        }
...
pub fn calculate_etas_parallel(
    ...
    if sequence.targets.len() > 10 {
        let results: Vec<_> = sequence.targets
            .par_iter()
            .enumerate()
            .map(|(idx, target)| {
                let runtime = target.runtime(download_time);
...
pub fn calculate_visibility_parallel(
    ...
    targets
        .par_iter()
        .map(|target| {
            let window = calculate_visibility_window(&target.coordinates, location, date, min_altitude);
...
pub fn get_schedule_info(
    ...
    sequence.targets
        ...
        .map(|target| {
            let window = calculate_visibility_window(&target.coordinates, location, date, 20.0);
            let quality = if window.is_visible {
                calculate_observation_quality(&target.coordinates, location, window.max_altitude_time)
            } else {
...
mod tests {
    ...
    fn test_merge_sequences() {
        ...
        assert_eq!(merged.targets.len(), seq1.targets.len() + seq2.targets.len());
    }
